...
class PoiDataset(Dataset):
    ...
    def __init__(self, users, times, time_slots, coords, locs,sequence_length, batch_size, split, usage, loc_count,
        ...
        self.timefreq=[]
        self.lbl_timefreq=[]
        self.sequences_timefreq=[]
        self.sequences_lbl_timefreq=[]
        ############################################################################
        ...
        self.capacity = 0
        for i, (time, time_slot, coord, loc, label, lbl_time, lbl_time_slot, lbl_coord) in enumerate(
            zip(self.times, self.time_slots, self.coords, self.locs, self.labels, self.lbl_times,self.lbl_time_slots,
                self.lbl_coords)):
            ...
            for j in range(seq_count): ## 这里 存放 单个用户的 sequence list
                ...
                #assert len(label[start:end])==len(lbl_time[start:end])==len(lbl_time_slot[start:end])==len(lbl_timefreq[start:end])==len(lbl_coord[start:end])==20
    ...
    def __getitem__(self, idx):
        ...
        return x, t, t_slot, s, y, reset_h, torch.tensor(self.active_users)
