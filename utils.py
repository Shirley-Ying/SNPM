...
from scipy.sparse import csr_matrix, coo_matrix, identity, dia_matrix, lil_matrix
...
def compute_similarity(graph):
    col = graph.col
    user_count = graph.shape[0]
    threshold = col.size // user_count

    bar = tqdm(total=user_count)
    bar.set_description('Construct user similarity')

    user_similarity_graph = lil_matrix((user_count, user_count), dtype=np.float32)
    for i in range(user_count):
        user_i_indices = col[i * threshold: (i + 1) * threshold]  # 第i个user所访问的top_k个poi
        user_i_indices = set(user_i_indices.tolist())
        sim_list = []
        for j in range(user_count):
            if i != j:
                user_j_indices = col[j * threshold: (j + 1) * threshold]  # 第i个user所访问的top_k个poi
                user_j_indices = set(user_j_indices.tolist())
                # 计算IOU指标
                sim = len(user_i_indices.intersection(user_j_indices)) / len(user_i_indices.union(user_j_indices))
                sim_list.append(sim)
        sim_array = np.array(sim_list)  # [0, 1, ..., i-1, i+1, ...]
        indices = np.argsort(sim_list)[::-1][:threshold]  # 取前top_k个最相似用户
        norm = np.max(sim_array[indices]) + 1e-8  # 避免除0
        for index in indices:
            if index < i:
                user_similarity_graph[i, index] = sim_array[index] / norm
            else:
                user_similarity_graph[i, index + 1] = sim_array[index] / norm
        bar.update(1)
    bar.close()
    with open('./KGE/new_gowalla_scheme1_transh_user_100.pkl') as f:
...
if __name__ == '__main__':
    with open('./KGE/gowalla_scheme1_transh_user-loc_100.pkl', 'rb') as f:
        transition_graph = pickle.load(f)  # 在cpu上

    transition_graph = coo_matrix(transition_graph)
    compute_similarity(transition_graph)
